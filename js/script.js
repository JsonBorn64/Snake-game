...
let gameField_width = /* gameField.clientWidth */ 360
let gameField_height = /* gameField.clientHeight */ 400
...
nickname.addEventListener('click', () => {
    nickname.textContent = prompt("Введите свой ник", localStorage.getItem('nickname'))
    if (nickname.textContent == '') {
        while (nickname.textContent == '') {
            nickname.textContent = prompt("Без ника нельзя", 'Ленивое создание')
        }
    }
...
if (!localStorage.getItem('nickname')) {
    nickname.textContent = prompt("Введите свой ник")
    if (nickname.textContent == '') {
        while (nickname.textContent == '') {
            nickname.textContent = prompt("Без ника нельзя", 'Ленивое создание')
        }
...
MenuButton.addEventListener('click', () => {
    ...
    if (MenuButton.textContent == 'Закрыть меню') {
...
const getData = () => {
    fetch('https://snake-49526-default-rtdb.europe-west1.firebasedatabase.app/db.json')
    .then(res => res.json())
    .then(data => {
        // Check and fill db
